...
shinyUI(
  ...
  mainPanel(
    aceEditor("ace", selectionId = NULL, cursorId="selection",value="createData <- function(rows){
  data.frame(col1=1:rows, col2=rnorm(rows))
}")
  )
))

#runApp(appDir = "D:/libraries/shinyAce/shinyAce/inst/examples/01-basic", display.mode = "showcase")
